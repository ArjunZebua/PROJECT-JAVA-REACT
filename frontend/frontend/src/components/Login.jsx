...
const Login = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const possibleUrls = [
        '/api/auth/login',              // Jika backend dan frontend same domain
        'http://localhost:8080/api/auth/login',  // Spring Boot default port
        'http://localhost:8081/api/auth/login',  // Alternative port
        '/api/login'                    // Original endpoint
      ];
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8">
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="text-xs text-gray-600 space-y-1">
            <p>Create an account first using the register form</p>
            <p>Backend URL: <code>/api/auth/login</code></p>
          </div>
        </div>
      </div>
    </div>
